...
public interface GetFlickrData extends RemoteService {
	...
	public static class App {
		...
		public static synchronized GetFlickrDataAsync getInstance() {
			if (app == null) {
				
				// set up a client side asynchronous class that connects with 
				// the FeedTempService service we've defined in GwtMini.gwt.xml 
				
				app = (GetFlickrDataAsync) GWT.create(GetFlickrData.class);
				((ServiceDefTarget) app).setServiceEntryPoint(GWT.getModuleBaseURL() + "/GetFlickrData");
			}
			return app;
		}
	}
